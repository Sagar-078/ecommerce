...
const OrderProgressBar = ({lastupdated,createdon,status,cancelhandler}:props) => {
  const steps=[
            ...
            status==="CANCELLED"?(<div>
              ...
              }
              {
                (status==="CONFIRMED" || status==="SHIPPED" )   && (<>
                       <p>Expected to be deliverd by {date.format((date.addDays(new Date(createdon), +2)), 'YYYY/MM/DD  ')} by 10 pm</p>
                </>)
              }
              </div>
            )
